...
const Products = () => {
  ...
  async function updateProducts(newQuery = query) {
    ...
    if (!arrayEquals(current, currentProducts))
      await setCurrentProducts(current);
  }
  ...
  async function setPage(pageNum) {
    ...
    history.replace(
      location.pathname + `?page=${pageNum}&limit=${query.limit}`
    );
  }
  ...
  function getCurrentProducts(newQuery) {
    ...
    const curr = products.filter((product) => {
      if (search.isSearch && !isCloseMatch(product.name, search.searchTerm))
        return false;
      if (
        price.isPrice &&
        !isBetween(product.price, price.minPrice * 100, price.maxPrice * 100)
      )
        return false;
      if (
        stock.isStock &&
        !isBetween(product.stock, stock.minStock, stock.maxStock)
      )
        return false;
      if (
        rating.isRating &&
        !isBetween(product.rating, rating.minRating, rating.maxRating)
      )
        return false;
      return true;
    });
  ...
  function isCloseMatch(str, searchTerm) {
    ...
    for (let term of searchTerm.split(' '))
      if (fmt.includes(term.toLowerCase())) return true;
    return false;
  }
    ...
    <div className='product-page'>
      ...
      <div className='product-container'>
        ...
        <PageSelector
          ...
          query={query}
          currentCount={
            currentProducts === undefined ? 0 : currentProducts.length
          }
        />
      </div>
    </div>
